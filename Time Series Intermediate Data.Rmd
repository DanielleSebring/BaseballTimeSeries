...
Our initial data source has restricted easy access to its database. As a result, we searched for a database with similar content and found it in *Lahman's Baseball Database*. This database is much cleaner and provides the potential 
for in-depth analysis down to an individual player-by-player level.

Initial data manipulation required aggregating the player level statistics to the league and team level. Some statistics present in our previous data source (e.g. batting average) were not present in Lahman's, so we had to do some calculations in order to maintain the consistency. After all data manipulation and cleanup, 
we have the following statistics for the years 1871-2021:
