...
bat_league <- batting %>% 
  ...
  mutate(BA = H/AB,
         ...
         OPS = OBP+SLG,
         R.G = R/G) %>%
  ## Add "per game" statistics for counts
  mutate(AB.G = AB/G,
         H.G = H/G,
         X2B.G = X2B/G,
         X3B.G = X3B/G,
...
We also notice that this time series has many spikes in it that deviate from the overall trend. This is likely due to modifications to the MLB schedule. For instance, the 2020 MLB season was drastically shortened due to COVID-19. Instead of playing a full slate of 162 games, each team only played 60 games. This naturally leads to a lower overall number of strikeouts. Other causes of this behavior could be a lockout due to negotiations between players and owners or times of war. Later on, we will show that these time series lead to many outliers in our fitted model. As a result, our assumption of normality is violated. One way to address this, and account for these outliers, is to average these statistics by the number of games.

```{r eda2, echo = FALSE}
par(mfrow = c(1,2))
plot(y = bat_league$H.G, x = bat_league$yearID, type = "l", xlab = "Year", 
     ylab = "Hits Per Game")
plot(y = pitch_league$SO.G, x = pitch_league$yearID, type = "l", xlab = "Year", 
